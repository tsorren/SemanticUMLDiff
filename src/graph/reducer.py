...
def _build_merged_graph(base: UMLModel, pr: UMLModel) -> nx.Graph:
...
def _identify_seeds_and_highlights(diff: DiffResult) -> Tuple[Set[str], Dict[str, str]]:
    ...
    return seed_nodes, highlight_dict


def _expand_context(graph: nx.Graph, seed_nodes: Set[str], context_depth: int) -> Set[str]:
...
def _apply_impacted_status(included_nodes: Set[str], highlight_dict: Dict[str, str]) -> None:
...
def _filter_relations(base: UMLModel, pr: UMLModel, included_nodes: Set[str]) -> Set[UMLRelation]:
